...
### 헤더에 클라우드 새로고침 버튼 추가
- 차트 숨기기 왼쪽에 `새로고침` 버튼을 추가했다.
- 이 버튼은 현재 세션 기준으로 클라우드 데이터를 다시 읽어와 PC와 모바일 간 동기화가 어긋난 경우 최신 상태를 재적용하는 용도다.
- 새로고침 확인 팝업도 가져오기와 동일하게 기준 시각을 표시한다.

영향 범위
- `index.html` 상단 글로벌 액션 버튼
- `js/app.js` 클라우드 재동기화 진입점

검증
- `node --check js/app.js` 통과

### 자동 백업 기준과 시세 강제 동기화 분리
- 자동 백업 보관 개수를 최근 20개로 확장했다.
- MTM 수동 수정과 FX 변경은 자동 백업 대상으로 유지하되, 시세 강제 동기화 자체는 자동 백업 트리거에서 제외했다.

영향 범위
- `js/app.js` 자동 백업 보관 개수와 트리거 기준
- `AGENTS.md` 자동 백업 운영 규칙
...
### 대시보드 기본 정렬과 포지션누적 기준 조정
- Trade History와 Performance Report의 기본 정렬을 `입력순` 내림차순으로 바꿨다.
- `포지션누적(만기)`는 같은 `상품/만기` 키 안에서 입력 순서대로 누적하되, 포지션 종료 시점에 0으로 리셋하지 않도록 바꿨다.

영향 범위
- `js/app.js` 테이블 기본 정렬 상태
- `js/app.js` `positionPnlCur` 순차 누적 계산
...
### 동기화 가져오기/내보내기 팝업에 기준 시각 추가
- 동기화 가져오기와 동기화 내보내기 확인 팝업에 기준 시각을 함께 표시했다.
- 가져오기는 원격 `blotter_meta.last_local_input_at`, 내보내기는 현재 로컬 입력 기준 시각을 보여준다.

영향 범위
- `js/app.js` 동기화 확인 팝업 문자열
- `js/app.js` 기준 시각 포맷 헬퍼
...
- Performance Report 상세 표에서 `포지션누적(만기)`를 `가상 미실현 손익(행손익)`보다 앞에 배치했다.
- 헤더와 행 렌더링 순서를 함께 맞춰 표시 위치만 변경했다.

영향 범위
- `index.html` Performance Report 테이블 헤더
- `js/app.js` Performance Report 행 렌더링

검증
- 미실행

### 퍼포먼스 리포트 포지션누적을 순번 누적으로 변경

- Performance Report의 `포지션누적(만기)`를 같은 `상품/만기` 구간 안에서 입력 순번에 따라 누적되는 값으로 바꿨다.
- 기존의 현재 포지션 최종 합계 표시에서 벗어나, 리포트 행이 진행될수록 누적값이 쌓이도록 계산 경로를 바꿨다.

영향 범위
- `js/app.js` Performance Report 행별 `positionPnlCur` 계산
- `AGENTS.md` 계산 규칙

검증
- 미실행

### 대시보드 Trade History에도 포지션누적 순차 누적 반영
...
### 퍼포먼스 리포트 가상손익과 포지션누적 순서 조정

- 퍼포먼스 리포트 상세 표에서 `가상 미실현 손익(행손익)`을 `포지션누적(만기)` 앞에 배치했다.
- 헤더와 본문 셀의 의미 대응이 더 자연스럽게 보이도록 열 순서를 맞췄다.

영향 범위
- `index.html` 퍼포먼스 리포트 헤더
- `js/app.js` 퍼포먼스 리포트 행 렌더링 및 정렬 라벨
