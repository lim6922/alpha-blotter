...
async function fetchYahooPrice(ySymbol) {
  ...
  // 1순위: CORSProxy 권장 ?url= 형식. 요청당 hard timeout 적용.
  try {
  ...
  // 2순위: AllOrigins fallback. 1순위 지연이 전체 갱신을 붙잡지 않도록 동일 timeout 적용.
  try {
...
/**
 * 시세 동기화 메인 함수
 * - 요청별 3.5초 hard timeout
 * - 최대 2개 제한 병렬 처리
 * - 종목별 실패 격리 및 기존 LAST 가격 유지
 */
async function syncMarketPrices() {
  ...
  try {
    ...
    if (priceEl && currentLast != null) {
      priceEl.value = currentLast;
    }
